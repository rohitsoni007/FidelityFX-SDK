<!-- @page page_techniques_frame-interpolation-swap-chain AMD FidelityFX™ Super Resolution Frame Generation Swapchain 3.1.7 -->

<h1>AMD FidelityFX™ Super Resolution Frame Generation Swapchain 3.1.7</h1>
...
The `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` implements the `IDXGISwapChain4` interface to provide an easy way to handle dispatching the workloads required for frame interpolation and pacing presentation.

Though this implementation may not work for all engines or applications, it was designed to provide an easy way to integrate `AMD FSR™ Frame Generation` in a way that it is (almost) transparent to the underlying application.

<h2>Description</h2>

`AMD FidelityFX™ Super Resolution Frame Generation Swapchain` can be used as a replacement of the APIs swapchain and from the application point of view behavior should be similar.

When `AMD FSR™ Frame Generation` is disabled, the main difference will be that present is slightly more expensive (one extra surface copy) compared to using the API swapchain directly.

In this case, the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` still supports handling the UI composition, so applications don't have to handle their UI differently when disabling `AMD FSR™ Frame Generation`.

Internally the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` will create 2 additional CPU threads:
...
![alt text](media/frame-interpolation-swapchain/frame-interpolation-swap-chain.svg "A diagram showing the CPU workflow of the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain`.")

<h2>Integration</h2>

The `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` should be integrated using the [AMD FidelityFX™ API](../getting-started/ffx-api.md), this document describes the API constructs specific to the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain`.

`AMD FidelityFX™ Super Resolution Frame Generation Swapchain` for DirectX 12 implements the `IDXGISwapChain4` interface, so once created it behaves just like a normal swapchain.
...
- Create and fill in a [`ffxCreateContextDescFrameGenerationSwapChainWrapDX12`](../../framegeneration/include/dx12/ffx_api_framegeneration_dx12.h#L34) structure then call [`ffxCreateContext`](../../api/include/ffx_api.h#L142). This will wrap & replace an existing swapchain.
- Create and fill in a [`ffxCreateContextDescFrameGenerationSwapChainNewDX12`](../../framegeneration/include/dx12/ffx_api_framegeneration_dx12.h#L42) structure then call [`ffxCreateContext`](../../api/include/ffx_api.h#L142). This will create a new swapchain from the given `dxgiFactory`.
- Create and fill in a [`ffxCreateContextDescFrameGenerationSwapChainForHwndDX12`](../../framegeneration/include/dx12/ffx_api_framegeneration_dx12.h#L52) structure then call [`ffxCreateContext`](../../api/include/ffx_api.h#L142). This will create a new swapchain from the provided `hwnd`, `desc`, `fullscreenDesc` and `dxgiFactory`. DX12 only allows one Swapchain to exist at a time for each hwnd, therefore the swapchain context has to be destroyed before a new swapchain is created for the hwnd.

<h3>Recording and dispatching the frame interpolation workload</h3>

The `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` has been designed to be independent of the AMD FidelityFX™ Frame Interpolation interface. To achieve this, it does not interact directly with those interfaces. The frame interpolation workload can be provided to the `FSR Frame Generation Swapchain` in 2 ways:

1. Provide a callback function [`frameGenerationCallback`](../../framegeneration/include/ffx_framegeneration.h#L121) in the [`ffxConfigureDescFrameGeneration`](../../framegeneration/include/ffx_framegeneration.h#L115).
   This function will get called from the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` during the call to `::Present` on the game thread, if frame interpolation is enabled to record the command list containing the frame interpolation workload.
2. Call [`ffxQueryDescFrameGenerationSwapChainInterpolationCommandListDX12`](../../framegeneration/include/dx12/ffx_api_framegeneration_dx12.h#L72) to obtain a command list from the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` and record the frame interpolation workload into it.
...
Note: to ensure presents can execute at the time intended by the `AMD FSR™ Frame Generation` frame pacing logic, avoid micro stuttering and assure good VRR response by the display, it is recommended to ensure the frame consists of multiple command list submissions.

<h3>UI Composition</h3>

When using `AMD FSR™ Frame Generation`, it is highly advisable to treat the UI with special care, since distortion due to game motion vectors that would hardly be noticeable in 3D scenes will significantly impact readability of any UI text and result in very noticeable artifacts, especially on any straight, hard edges of the UI.

To combat any artifacts and keep the UI nice and readable, there are 3 ways to handle UI composition in the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain`:
...
3. Provide a surface containing the HUD-less scene to the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` in addition to the final back buffer.
...
An application can get amount of GPU local memory required by `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` context after context creation by calling [`ffxQuery`](../../api/include/ffx_api.h#L160) with the valid context and [`ffxQueryFrameGenerationSwapChainGetGPUMemoryUsageDX12`](../../framegeneration/include/dx12/ffx_api_framegeneration_dx12.h#L108).

An application can get GPU local memory required by default `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` version before context creation by calling [`ffxQuery`](../../api/include/ffx_api.h#L160) with `NULL` context and filling out [`ffxQueryFrameGenerationSwapChainGetGPUMemoryUsageDX12V2`](../../framegeneration/include/dx12/ffx_api_framegeneration_dx12.h#L115). To get the memory requirement info for a different `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` version, additionally link [`ffxOverrideVersion`](../../api/include/ffx_api.h#L109). 

See code examples how to call [Query](../getting-started/ffx-api.md#Query).

<h3>Waitable Object</h3>

It is advisable that the game uses GetFrameLatencyWaitableObject to get a waitable object, then use that object to prevent the CPU from running too far ahead of the GPU. This is especially important when VSync is on at a low refresh rate monitor, as the GPU render rate can fall far below the CPU submission rate. Alternatively, the app can use a frame limiter set to half monitor refresh rate.

**Important:** **The application must close the returned handle** using `CloseHandle()` after destroying the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` context (via `ffxDestroyContext`). The handle remains valid when frame generation is toggled on/off via configuration without destroying the swapchain context. Each call to `GetFrameLatencyWaitableObject()` returns a **new independent handle** (via `DuplicateHandle` internally) that **must be closed separately** by the application.

```cpp
// Get waitable handle once (each call returns a new handle)
HANDLE waitableHandle = fgSwapchain->GetFrameLatencyWaitableObject();

// Use for frame pacing
WaitForSingleObject(waitableHandle, INFINITE);

// On shutdown: destroy context first, then close handle
ffx::DestroyContext(swapChainContext);
CloseHandle(waitableHandle);
```

<h3>Frame pacing and presentation</h3>

The `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` handles frame pacing automatically. Since Windows is not a real-time operating system and variable refresh rate displays are sensitive to timing imprecisions, the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` has been designed to use a busy wait loop in order to achieve the best possible timing behavior.

With `AMD FSR™ Frame Generation` enabled, frames can take wildly different amounts of time to render. The workload for interpolated frames can be much smaller than for application rendered frames ("real" frames). It is therefore important to properly pace presentation of frames to ensure a smooth experience. The goal is to display each frame for an equal amount of time.
...
This section covers questions about implementation details regarding pacing, expected behavior and recommendations learned from working with various developers integrating `AMD FSR™ Frame Generation`.

<h4>Backbuffer Count</h4>

Without `AMD FSR™ Frame Generation`, usually applications use double buffering when VSync is disabled or triple buffering with VSync enabled:
...
The `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` implements a swapchain with the amount of backbuffers specified by the application, plus a real swapchain, which supports triple buffering. From the application side it is **sufficient to always specify double buffering**. 
...
**To minimize input latency, the present call of the `AMD FidelityFX™ Super Resolution Frame Generation Swapchain` will wait for the previous frames interpolation and ui composition to be finished.**
...
As a result, the previous backbuffer will always be available for rendering the next frame once the present call returns, so the application can preserve some GPU memory by creating the swapchain with no more than 2 backbuffers, even with VSync enabled.
...
`AMD FSR™ Frame Generation 4.0.1` has been designed for an input framerate of at least 30 FPS.

If the frame rate of the application drops below 60FPS (FSR-SRFG 3.1.6) or 30FPS (FSR-FG 4.0.1) generated frames may result in visual artifacts severe enough to negatively impact the user experience. The exact framerate where artifacts become noticeable will depend on application, scene and movement of objects and camera.

`AMD FidelityFX™ Super Resolution Frame Generation Swapchain 3.1.7` has been designed to give optimal results with an input framerate of 60 FPS or higher. The implementation will function with framerates of at least 30 FPS, but frame-pacing may not be optimal.

Low framerates typically come with high input latency, so depending on the application, it may be preferable to recoup some performance by disabling  `AMD FSR™ Frame Generation` by setting the `ffx::ConfigureDescFrameGeneration::frameGenerationEnabled` to `false` if the frame rate consistently drops below that threshold. 
This can help performance in both CPU and GPU limited scenarios, since interpolation happens on the GPU and disabling `AMD FSR™ Frame Generation` will also disable the busy wait loops in the pacing logic and free up CPU resources. 

It is recommended to re-compute the average frametime for a couple of seconds after enabling or disabling `AMD FSR™ Frame Generation` and compute the performance difference from disabling `AMD FSR™ Frame Generation`.
Reverting the decision to enable/disable `AMD FSR™ Frame Generation` should only be done if the framerate changes more than what can be attributed to the performance difference from disabling it.
...
- If the application could run significantly faster than half the monitor refresh rate, `AMD FSR™ Frame Generation` will slow down the application to half refresh rate, so every interpolated and real frame gets displayed for one refresh. In this case disabling `AMD FSR™ Frame Generation` or VSync can result in less input latency.
...
Tearing artifacts may appear even with VRR enabled and the application presenting within the VRR window.
...
Then call [`ffxDestroyContext`](../../api/include/ffx_api.h#L148) on the proxy swap chain context.
...
- One asynchronous compute queue  - only used when [`FFX_FRAMEGENERATION_ENABLE_ASYNC_WORKLOAD_SUPPORT`](../../framegeneration/include/ffx_framegeneration.h#L40) is set on FSR3 context creation and [`allowAsyncWorkloads`](../../framegeneration/include/ffx_framegeneration.h#L124) is true in the [`ffxConfigureDescFrameGeneration`](../../framegeneration/include/ffx_framegeneration.h#L115).
